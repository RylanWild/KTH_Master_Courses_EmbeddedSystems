...
procedure mixedscheduling is
   ...
   Start : Time; -- Start Time of the System
	Calibrator: constant Integer := 900; -- Calibration for correct timing  best at 1000 original:850
   ...
   task type T(Id: Integer; Prio: Integer; Phase: Integer; Period : Integer; 
	...
	--Round Robin Task
	task type Round_Robin(Id: Integer; Prio: Integer; Phase: Integer; Period : Integer; Computation_Time : Integer) is
	  pragma Priority(Prio); -- background tasks are all on Priority Level 1
	end Round_Robin;
      ...
      loop
         ...
         -- Simulation of User Function
			for I in 1..Computation_Time loop
				Dummy := F(Calibrator); --this is where Calibrator is used
			end loop;	
			Completed := Clock;
   ...
   Task_1 : T(1, 20, Warm_Up_Time, 300, 100, 300); -- ID: 1
                                                      ...
                                                      --	Phase: Warm_Up_Time (100)
	                                                   ...
	                                                   -- Relative Deadline: 200
	
-- Main Program: Terminates after measuring start time	

	Task_2 : T(2, 19, Warm_Up_Time, 400, 100, 400); -- ID: 1
	                                                   -- Priority: 20
                                                      --	Phase: Warm_Up_Time (100)
	                                                   -- Period 2000, 
...
-- Main Program: Terminates after measuring start time	
	...
	Task_3 : T(3, 18, Warm_Up_Time, 600, 100, 600); -- ID: 1
	                                                   -- Priority: 20
                                                      --	Phase: Warm_Up_Time (100)
	                                                   -- Period 2000, 
	                                                   -- Computation Time: 1000 (if correctly calibrated) 
...
-- Round Robin Task
	Task_R1:Round_Robin(1,1,Warm_Up_Time,100,100);
	Task_R2:Round_Robin(2,1,Warm_Up_Time,100,100);
	Task_R3:Round_Robin(3,1,Warm_Up_Time,100,100);
begin
